    ...
    async execute() {
        ...
        try {
            ...
            for (let i = 0; i < items.length; i++) {
                const operation = this.getNodeParameter('operation', i, 'select');
            ...
            throw new Error(`DataTable error: ${error.message}`);
        }
        finally {
            if (db) {
                try {
...
exports.DataTableNode = DataTableNode;
...
function handleCreateTable(db, ctx, itemIndex, tableName) {
    const rawColumns = ctx.getNodeParameter('columns', itemIndex, {});
...
function handleInsert(db, ctx, itemIndex, items, tableName) {
    let rawData = ctx.getNodeParameter('data', itemIndex, '');
    const sanitizedName = sanitizeIdentifier(tableName);
    const result = [];
    let records;
    if (rawData) {
...
function handleSelect(db, ctx, itemIndex, tableName) {
    const rawWhere = ctx.getNodeParameter('whereClause', itemIndex, '');
    const sanitizedName = sanitizeIdentifier(tableName);
    let whereClause = '';
    let params = {};
    if (rawWhere) {
        try {
            params = JSON.parse(rawWhere);
        }
        catch {
...
function handleUpdate(db, ctx, itemIndex, tableName) {
    const rawData = ctx.getNodeParameter('data', itemIndex, '');
    const rawWhere = ctx.getNodeParameter('whereClause', itemIndex, '');
    const sanitizedName = sanitizeIdentifier(tableName);
    if (!rawData) {
    ...
    if (rawWhere) {
        try {
            whereParams = JSON.parse(rawWhere);
        }
        catch {
            throw new Error(`Invalid JSON in Where Clause: ${rawWhere}`);
        }
...
function handleDelete(db, ctx, itemIndex, tableName) {
    const rawWhere = ctx.getNodeParameter('whereClause', itemIndex, '');
    const sanitizedName = sanitizeIdentifier(tableName);
    let whereClause = '';
    let params = {};
...
//# sourceMappingURL=DataTableNode.node.js.map