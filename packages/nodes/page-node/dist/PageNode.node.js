...
class PageNode {
    ...
    async execute() {
        ...
        for (let i = 0; i < items.length; i++) {
            const pageTitle = this.getNodeParameter('pageTitle', i, 'My App');
            const layout = this.getNodeParameter('layout', i, 'default');
            const showNav = this.getNodeParameter('showNav', i, true);
...
function buildPage(title, layout, showNav, data) {
    const layouts = {
        ...
        <div class="overflow-x-auto bg-white rounded-lg shadow">
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ${keys.map((key) => `<th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">${escapeHtml(key)}</th>`).join('')}
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ${data.map((row, idx) => `
                <tr class="${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-gray-100">
                  ${keys.map((key) => `<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${escapeHtml(String(row[key] ?? ''))}</td>`).join('')}
...
//# sourceMappingURL=PageNode.node.js.map