...
class FormNode {
    ...
    async execute() {
        ...
        for (let i = 0; i < items.length; i++) {
            const rawFields = this.getNodeParameter('formFields', i, {});
            const fields = rawFields?.fields || [];
            const submitText = this.getNodeParameter('submitText', i, 'Submit');
            const successMessage = this.getNodeParameter('successMessage', i, 'Form submitted successfully!');
            const formAction = this.getNodeParameter('formAction', i, '');
...
function isFormRenderRequest(data) {
    // Has route key = App Router routed output = render form
    if ('route' in data && typeof data.route === 'object')
        return true;
...
function buildFormHtml(fields, submitText, successMessage, formAction, csrfToken) {
...
function renderField(field) {
    ...
    switch (field.type) {
        ...
        case 'select': {
            ...
            const optionHtml = options.map((opt) => `<option value="${escapeHtml(opt)}">${escapeHtml(opt)}</option>`).join('\n');
            return `
        <div>
          ${labelHtml}
...
//# sourceMappingURL=FormNode.node.js.map